...
class ershouSpider(Spider):
	...
	def start_requests(self):
		...
		if debug and os.path.isfile(parent_path+'/lianjia/result/ershouurls.json') and os.path.isfile(parent_path+'/lianjia/result/ershou.json'):
			checked = []
			with open(parent_path+'/lianjia/result/ershou.json','rb') as f:
				for line in f:
					url = json.loads(line)['url']
					url = filter(lambda ch: ch in '0123456789',url)
					checked.append(url)	
			with open(parent_path+'/lianjia/result/ershouurls.json','rb') as f:
				for line in f:
